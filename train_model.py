...
from pycaret.classification import setup as cls_setup, compare_models, finalize_model, save_model, plot_model
from pycaret.clustering import setup as clu_setup, create_model as clu_create_model, assign_model as clu_assign_model, save_model as clu_save_model, plot_model as clu_plot_model
...
def train_classifier(df: pd.DataFrame, feature_cols):
    ...
    plot_model(final_best, plot="confusion_matrix", save=True); save_plot_safely("confusion_matrix.png")
    plot_model(final_best, plot="auc", save=True); save_plot_safely("roc.png")
    plot_model(final_best, plot="pr", save=True); save_plot_safely("pr.png")
    plot_model(final_best, plot="feature", save=True); save_plot_safely("feature.png")
...
def train_clusterer(df_malicious, feature_cols):
    ...
    try:
        clu_plot_model(kmeans, plot="elbow", save=True); save_plot_safely("elbow.png")
        clu_plot_model(kmeans, plot="silhouette", save=True); save_plot_safely("silhouette.png")
        clu_plot_model(kmeans, plot="cluster", save=True); save_plot_safely("cluster.png")
    except Exception as e:
        print(f"[!] Plot error: {e}")

    # Map clusters -> profiles using centroid heuristics
    centroids = labeled.groupby("Cluster")[feature_cols].mean()
    def s_state(c): return 1.5*c["SSLfinal_State"] + 1.0*c["Prefix_Suffix"] + 0.8*(c["domain_age_days"]/4000.0) - 1.0*c["Shortining_Service"] - 1.0*c["having_IP_Address"] - 0.5*c["suspicious_tld"] - 0.5*c["redirect_count"]/10.0
    def s_crime(c): return 1.2*c["Shortining_Service"] + 1.2*c["having_IP_Address"] + 1.0*c["abnormal_url"] + 1.0*c["suspicious_tld"] + 0.7*(c["url_length"]/200.0) + 0.7*(c["num_subdomains"]/5.0) + 0.7*(c["redirect_count"]/10.0) - 0.5*c["SSLfinal_State"]
    def s_hackt(c): return 1.5*c["has_political_keyword"] + 0.8*c["abnormal_url"] + 0.3*c["SSLfinal_State"] + 0.2*(c["domain_age_days"]/4000.0)
    ...
    for cl, r in df_scores.iterrows():
        triples += [(cl, "State-Sponsored", r["State"]), (cl, "Organized Cybercrime", r["Crime"]), (cl, "Hacktivist", r["Hackt"])]
    triples.sort(key=lambda t: t[2], reverse=True)
    used = set()
    for cl, profile, _ in triples:
        if cl not in used and profile in remaining:
            cluster_map[cl] = profile
            used.add(cl); remaining.remove(profile)
        if len(cluster_map)==3: break
...
def main():
    ...
    feature_cols = ["SSLfinal_State","Prefix_Suffix","Shortining_Service","having_IP_Address","abnormal_url","has_political_keyword","suspicious_tld","url_length","num_subdomains","redirect_count","domain_age_days","https_token_in_url"]
